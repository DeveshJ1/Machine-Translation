...
def run_model_eval(beam_search=False, beam_size=4, verbose=True, num_ex=None):
    ...
    #commented code below was for 2.4.2 to test beam and greedy were giving same results and for submitting out_beam.txt for beam_size=2
    '''
    results_greedy, score_greedy = eval_model(
        valid_dataloader, model, vocab_src, vocab_tgt, beam_search=False, beam_size=1,
    )


    results_beam, score_beam = eval_model(
        valid_dataloader, model, vocab_src, vocab_tgt, beam_search=True, beam_size=1,
    )
     results_beam, score_beam = eval_model(
        valid_dataloader, model, vocab_src, vocab_tgt, beam_search=True, beam_size=2,
    )
    print(score_beam)
    print(score_greedy)
    '''
    #commented code below was for 2.4.3 to plot the beam size vs scores relationship
    '''
    beam_sizes = list(range(1, 6))  # [1, 2, 3, 4, 5]
    scores = []

    for beam_size in beam_sizes:
      # Evaluate with current beam size
      results_beam, score_beam = eval_model(
        valid_dataloader,
        model,
        vocab_src,
      ...
      scores.append(score_beam)
      print(f"Beam size: {beam_size}, BLEU score: {score_beam}")
